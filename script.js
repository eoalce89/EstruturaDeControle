...
class Particula {
    ...
    atualizar(){
        ...
        if(this.direcaoY > canvas.height || this.y < 0){
...
function conectar(){
    for(let a = 0; a < particulasArray.length; a++){
        for(let b = a; b < particulasArray.length; b++){
            let distancia = (
                (particulasArray[a].x - particulasArray[b].x) * (particulasArray[a].x) + ( (particulasArray[a].y)) - (particulasArray[b].x)) + ((particulasArray[a].y) - (particulasArray[b].y));
...
window.addEventListener('realize', (event) => {
    canvas.width = window.innerWidth;
    canvas.height = window.innerHeight;
    int();
})
